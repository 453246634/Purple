...
function hashMerchantPwd(pwd) {
  return crypto.createHash('sha256').update('zima_pay_' + pwd + '_security_2024').digest('hex');
}

function hashMerchantPwdUid(pwd) {
  return crypto.createHash('sha256').update('zima_pay_uid_' + pwd + '_security_2024').digest('hex');
}
...
app.post('/g/:groupId/cashier/qrcode', express.json(), async (req, res) => {
    ...
    if (rt.alipaySdk) {
      try {
        ...
        const result = await rt.alipaySdk.exec('alipay.trade.precreate', {
          bizContent: { out_trade_no: outTradeNo, total_amount: amtNum.toFixed(2), subject, body, timeout_express: '30m', notify_url: notifyUrl },
        });
        ...
        if (resp.code === '10000' && tradeQrCode) {
          ...
          const order = { outTradeNo, amount: amtNum.toFixed(2), subject, status: 'generated', createdAt: new Date().toISOString(), paidAt: null, payerIp: getClientIp(req), useTradeApi: true, groupId: req.group.id, slotIndex };
          rt.orders.push(order);
  ...
  try {
    ...
    if (resp.code === '10000' && qrCode) {
      ...
      setTimeout(() => rt.cashierOrders.delete(outTradeNo), 31 * 60 * 1000);
      const order = { outTradeNo, amount: amtNum.toFixed(2), subject, status: 'generated', createdAt: new Date().toISOString(), paidAt: null, payerIp: getClientIp(req), groupId: req.group.id, slotIndex };
      rt.orders.push(order);
      saveMerchantFile(m.id, 'orders', rt.orders);
...
function handleAlipayRedirect(req, res, next) {
  ...
  if (/alipayclient|alipaydefined|aliapp/i.test(ua)) {
    ...
    // 仅 uid / uid-simple 类型商户支持 302 跳转
...
// 服务端注入商户名称到 cashier.html（比前端 fetch 更可靠）
app.use('/m/:id', (req, res, next) => {
...
app.post('/m/:id/cashier/qrcode', express.json(), async (req, res) => {
  ...
  if (m.type === 'uid' || m.type === 'uid-simple') {
    ...
    if (rt.alipaySdk) {
      try {
        ...
        if (resp.code === '10000' && tradeQrCode) {
          ...
          const order = { outTradeNo, amount: amtNum.toFixed(2), subject, status: 'generated', createdAt: new Date().toISOString(), paidAt: null, payerIp: getClientIp(req), useTradeApi: true };
          rt.orders.push(order);
          saveMerchantFile(m.id, 'orders', rt.orders);
          ...
          if (m.type === 'uid-simple') {
            ...
            try { qrDataUrl = await QRCode.toDataURL(tradeQrCode, { width: 300, margin: 2, color: { dark: '#000000', light: '#ffffff' } }); } catch (e) {}
            return res.json({
              code: 'OK', out_trade_no: outTradeNo, qr_code: tradeQrCode, qr_image: qrDataUrl,
    ...
    if (m.type === 'uid-simple') {
      ...
      return res.json({
        code: 'OK', out_trade_no: outTradeNo, qr_code: alipaysUrl, qr_image: qrDataUrl,
  ...
  try {
    ...
    if (resp.code === '10000' && qrCode) {
      ...
      const order = { outTradeNo, amount: amtNum.toFixed(2), subject, status: 'generated', createdAt: new Date().toISOString(), paidAt: null, payerIp: getClientIp(req) };
      rt.orders.push(order);
      saveMerchantFile(m.id, 'orders', rt.orders);

      // 启动自动确认（后台轮询 trade.query + 账单兜底），不依赖前端页面保持打开
      startAutoConfirm(m.id, rt, order);
...
app.post('/m/:id/cashier/notify', express.urlencoded({ extended: false }), async (req, res) => {
  ...
  if (rt.alipaySdk) {
    try {
      const signVerified = rt.alipaySdk.checkNotifySign(req.body);
      if (!signVerified) {
        console.log('[支付宝通知] 签名验证失败: ' + outTradeNo);
        return res.send('fail');
      }
    } catch (e) {
      console.log('[支付宝通知] 签名验证异常: ' + e.message);
      return res.send('fail');
    }
  }
